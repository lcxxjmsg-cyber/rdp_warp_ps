...
$script:OFFSET_SUPPORT = @('dbghelp.dll','symsrv.dll','symsrv.yes')
$script:OFFSET_STORE = "$env:ProgramData\rdpwarp\symbols"
...
$script:BINARY_SHA256 = @{
    ...
    'RDPWrapOffsetFinder_x64.dll'='7214089BBBFBE218CD3A3BB759A26B407F3A24C74FBDD1284B2D2765555EA1E0'
    'RDPWrapOffsetFinder_x86.dll'='11621DA4B0DE251724FF911B4EC906DC9FDD8E9655811CDE77B736FFD9AE3BD7'
}
...
function Get-GitHubDownloadUrls {
    ...
    if ($Uri -match '^https://(raw\.githubusercontent\.com|github\.com)/') {
        $mirrors = New-Object System.Collections.Generic.List[string]
        if ($script:GH_MIRROR) { $script:GH_MIRROR -split ',' | ForEach-Object { $mirrors.Add($_.Trim().TrimEnd('/')) } }
        @('https://gh-proxy.com','https://ghproxy.net','https://ghfast.top') | ForEach-Object { $mirrors.Add($_) }
        foreach ($m in @($mirrors | Select-Object -Unique)) {
            if ($m) { $urls.Add("$m/$Uri") }
        }
    }
    if ($Uri) { $urls.Add($Uri) }
...
"@
        ...
        if ($ofl) {
            ...
            foreach ($sf in $script:OFFSET_SUPPORT) {
                $src = if ($script:BIN_DIR) { Join-Path $script:BIN_DIR $sf } else { $null }
                if ($src -and (Test-Path $src)) { Copy-Item $src "$script:RDPWRAP_DIR\$sf" -Force }
            }
            Write-S "OffsetFinder deployed ($script:OFFSET_DLL)"
        } else {
            Write-W "Compatible OffsetFinder DLL not found; OffsetFinder fallback is disabled"
        }
...
function Update-RdpwrapIni {
    ...
    if (-not (Test-Path "$script:RDPWRAP_DIR\$script:OFFSET_DLL")) {
        Write-W "No validated community entry; OffsetFinder not deployed, skipping generator fallback"
        return $false
    }
...
"@
        ...
        $hr = [RDPOffsetFinderNative]::FindRDPOffsetsNoSym($termsrv, $sb, $sb.Capacity, 0)
        $generated = $sb.ToString()
        if ($hr -eq 0 -and $generated -and -not $generated.Contains("ERROR:")) {
            ...
            if ($check.Valid) {
                ...
                Write-S "OffsetFinder generated a complete candidate for $ver; runtime verification pending"
                return $true
            }
            Write-W "OffsetFinder output rejected: $($check.Message)"
        } else {
            Write-W "OffsetFinder did not produce a clean result (HRESULT=$hr)"
        }
    } catch { Write-W "OffsetFinder failed: $_" }
...
public class F{[DllImport("$($d-replace'\\','\\\\')",CharSet=CharSet.Unicode)]public static extern int FindRDPOffsetsNoSym(string p,StringBuilder o,int s,int f);}
"@;$b=New-Object Text.StringBuilder 131072;$h=[F]::FindRDPOffsetsNoSym($t,$b,$b.Capacity,0);$g=$b.ToString();if($h-eq0-and$g-and!$g.Contains("ERROR:")){$base=Remove-IniSection $c $k;$base=Remove-IniSection $base "$k-SLInit";$x="$($base.TrimEnd())`r`n`r`n$g`r`n";$q=Test-RdpIniCandidate -Content $x -Version $k;if($q.Valid){Copy-Item $i "$i.bak" -Force -EA 0;$x|Out-File $i -Encoding ASCII;$ok=$true;w"Validated OffsetFinder candidate"}else{w"Rejected OffsetFinder output: $($q.Message)"}}}catch{w"OffsetFinder failure: $_"}}
...
function Invoke-RdpShadow {
    param(
        ...
        [string]$Password = '',
        [switch]$ForceConsent,
        [switch]$ForceNoConsent,
        [switch]$ViewOnly
    )
    $id = [Security.Principal.WindowsIdentity]::GetCurrent()
    if (-not (New-Object Security.Principal.WindowsPrincipal($id)).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
        Write-E '影子需要管理员（提升令牌）。请右键"以管理员身份运行"本工具。'
    ...
    $targets = if ($isRemote) {
        if ($User) { @(Get-RdpRemoteSessions -Server $targetHost) } else { @() }
    } else {
    ...
    if (-not $isRemote) {
        ...
        if ($val -eq 0) { Write-W '策略为"禁用"，不发起影子。'; cmd /c pause 2>&1 | Out-Null; return }
        $map = @{0='禁用';1='完全/需同意';2='完全/免同意';3='查看/需同意';4='查看/免同意'}
        Write-I "目标=$($t.User)  有效策略=$($mode.Value)($($map[$mode.Value]))  来源=$($mode.Source)"
    } else {
        if ($ViewOnly) { $fullControl = $false }
        if ($ForceConsent) { $noConsent = $false }
        Write-I "目标主机=$targetHost  远端会话 ID=$SessionId"
        if ($User) { Write-I "已缓存凭据 for $targetHost (cmdkey)" }
    }
